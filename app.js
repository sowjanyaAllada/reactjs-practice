...
const getFollowingPeopleUser = async username => {
  const getFollowingPeopleQuery = `SELECT following_user_id FROM followerINNER JOIN user ON user.user_id=follower.follower_user_id WHERE user.username='${username}';`
...
const tweetAccessVerification = async (request, response, next) => {
  ...
  const getTweetQuery = `SELECT * FROM tweet INNER JOIN follower ON tweet.user_id=follower.following_user_id WHERE tweet.tweetId='${tweetId}' AND follower_user_id='${userId}';`
...
app.get('/user/tweets/feed/', authentication, async (request, response) => {
  ...
  const followingPeoppleIds = await getFollowingPeopleUser(username)
  const getTweetsQuery = `SELECT username,tweet,date_time as dateTime FROM user INNER JOIN tweet ON user.user_id=tweet.user_id WHERE 
    user.user_id IN (${followingPeoppleIds}) ORDER BY date_time DESE LIMIT 4;`
...
app.get(
  ...
  async (request, response) => {
    ...
    const likesUsers = await db.get(getlikesQuery)
...
app.get(
  ...
  async (request, response) => {
    ...
    const repliedUsers = await db.get(getrepliedQuery)
...
app.get(
  ...
  async (request, response) => {
    ...
    const getTweetsQuery = `SELECT tweet,
                       ...
                       date_time AS dateTime
                      FROM tweet LEFT JOIN reply ON tweet.tweet_id=reply.tweetId
                      LEFT JOIN like ON tweet.tweet_id=like.tweetId
